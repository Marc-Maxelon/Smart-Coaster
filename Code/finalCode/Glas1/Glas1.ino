...
Adafruit_MQTT_Publish feedGlas = Adafruit_MQTT_Publish(&mqtt, AIO_USERNAME "/feeds/glas-1");
Adafruit_MQTT_Publish feedGewicht = Adafruit_MQTT_Publish(&mqtt, AIO_USERNAME "/feeds/gewicht-1");
Adafruit_MQTT_Publish feedError = Adafruit_MQTT_Publish(&mqtt, AIO_USERNAME "/feeds/error");
Adafruit_MQTT_Publish feedEmpty = Adafruit_MQTT_Publish(&mqtt, AIO_USERNAME "/feeds/leer-warnung");
...
void loop() {
  ...
  // Wir checken nur, wenn kein Glas bekannt ist oder wir ein neues erzwingen wollen
  if (mfrc522.PICC_IsNewCardPresent() && mfrc522.PICC_ReadCardSerial()) {
    ...
    if (mfrc522.MIFARE_Read(PAGE_ADDR, buffer, &size) == MFRC522::STATUS_OK) {
      ...
      // Werte in Gramms
      if (drinkID == 1)      { currentDrink = "Softdrink"; currentGlassW = 393.0; currentMaxL = 330.0; }
      else if (drinkID == 2) { currentDrink = "Wein";      currentGlassW = 162.0; currentMaxL = 200.0; }
      else if (drinkID == 3) { currentDrink = "Bier";      currentGlassW = 180.0; currentMaxL = 250.0; }
  ...
  if (millis() - lastReportTime > reportInterval) {
    ...
    // FALLS wir ein Glas haben und das Gewicht passt:
    ...
    // FALLS kein Glas da ist und kein RFID erkannt wurde -> Error zeigen
        ...
        // Wir senden den Fehler nur, wenn die Waage auch wirklich leer ist
        if (currentRaw < 50.0) { 
